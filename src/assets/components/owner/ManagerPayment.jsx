...
export default function ManagerPayment() {
  ...
  const fetchAllTodayPayments = async () => {
    try {
      const res = await fetch("https://error-404-server.onrender.com/api/payment/today/all");
  ...
  const handleCollectPaymentForSession = async (session) => {
    ...
    try {
      const res = await fetch(
        "https://error-404-server.onrender.com/api/payment/collect-multiple",
        {
          method: "PUT",
  ...
  return (
    <div className="manager-payment-container">
      <h2 className="title">Owner Payment Panel</h2>
