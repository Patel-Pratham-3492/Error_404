...
export default function Track({ table }) {
  ...
  const fetchOrders = async () => {
    if (!table) return;
    try {
      const res = await fetch(`https://error-404-server.onrender.com/api/order/table/${table._id}`);
  ...
  const handleUpdateOrder = async () => {
    ...
    try {
      const res = await fetch(`https://error-404-server.onrender.com/api/order/${_id}/update-item`, {
        method: "PUT",
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      const res = await fetch(`https://error-404-server.onrender.com/api/order/${orderId}`, {
        method: "DELETE",
      });
      const data = await res.json();
