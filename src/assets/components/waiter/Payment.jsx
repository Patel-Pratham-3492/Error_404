...
export default function Payment({ table }) {
  ...
  const fetchPayments = async () => {
    ...
    try {
      const res = await fetch(`http://3.128.94.231:5000/api/payment/table/${table.customerCount}`);
  ...
  const handleCollectPayment = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://3.128.94.231:5000/api/payment/collect-multiple", {
        method: "PUT",
